...
class TextEncoder(nn.Module):
    ...
    def forward(self, prompts, tokenized_prompts):
        # Figure out what happens here, how are prompts/tokenized prompts used
        # Check dimensions of prompts, how do match? 93/77
...
class RegressionPrompter(nn.Module):
    def __init__(self, clip_model, coop_hyperparams):
        super().__init__()
        # n_cls = len(coop_hyperparams['classes'])
        dtype = clip_model.dtype
        ...
        tokenized_prompt = clip.tokenize(prompt).to(device_num)
        with torch.no_grad():
        ...
        self.tokenized_prompts = tokenized_prompt # torch.Tensor
...
class CLIPLinearProbe(nn.Module):
    def __init__(self, clip_model, coop_hyperparams):
        super().__init__()
        self.prompt_learner = RegressionPrompter(clip_model, coop_hyperparams)
    ...
    def forward(self, image):
        ...
        scores = image_features @ text_features.t() + 4.5
...
class SONCLIPFewShotNet(nn.Module):
    def __init__(self, basenet, coop_hyperparams):
        super().__init__()
        self.coop_learner = CLIPLinearProbe(basenet, coop_hyperparams)
    ...
    def forward(self, img):
        ...
        return logits # pos_likelihood # scenicness
...
### General iteration functions ###
    ...
    def iteration_forward(self, batch, tracker, split):                
        preds = self.net(batch['img']) # (self.net(batch['img']) * 9) + 1 # Scale to 1-10 range
    ...
    def on_save_checkpoint(self, checkpoint):
        ...
        checkpoint['prompt_state'] = prompt_state
...
### Testing ###
    ...
    def configure_optimizers(self):
        optimizer = torch.optim.SGD(self.net.coop_learner.parameters(), lr=self.lr, weight_decay=self.decay)#, momentum=False)
